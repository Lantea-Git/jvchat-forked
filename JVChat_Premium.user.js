...
/* [NEW LAYOUT SCROLL RESPAWN 2 MSG ]*/
...
.jvchat-content .message__urlImg {
    height: 39px;
    width: 52px;
    display: inline-block;
...
#jvchat-risi-reduce-range,
#jvchat-risi-reduce-span,
#jvchat-citation-checkbox,
#jvchat-citation-span {
    cursor: pointer;
}

#bloc-formulaire-forum #risibank-container {
    max-height: var(--jvchat-risi-max-height);
}
...
.panel-body .titre-info-fofo {
    ...
    border-bottom: 0.0625rem solid #9c9da7;
...
.nav-link-search,
.account-pseudo,
.jv-account-number-mp,
.jv-account-number-notif {
...
.alert-success {
    --jv-alert-color: #0f5132;
    --jv-alert-bg: #d1e7dd;
    --jv-alert-border-color: #badbcc;
}
...
.alert {
    ...
    background-color: var(--jv-alert-bg);
    border: 0.0625rem solid var(--jv-alert-border-color);
...
function getTopicLocked(elem, payload) {
    ...
    try {
        ...
        } else if (payload?.forum?.lockReason?.post) { //Exclusion Forum fermé par exemple (Pas de préfix)
...
//Switch entre la zone de texte réduite (Press enter et textarea seulement) et le mode complet (Legacy interface)
...
function parseMessage(elem) {
    ...
    let classUser = [...conteneur.getElementsByClassName("bloc-pseudo-msg")[0].classList].pop();
...
function parseUserInfo(elem) {
    ...
    if (!accountMp) { // Pas d'icone MP === User Deconnecté = on return undefined et on skip.
            ...
            <div id='jvchat-config' class='jvchat-hide'>
                <div id='jvchat-configuration'>
                    ...
                    <div class="jvchat-config-option" id="jvchat-risi-reduce">
                        <label>
                            <span>Hauteur Max RisiBank</span>
                        </label>
...
// Récupération du bouton POST original et clone pour détacher les listeners natifs.
...
function clearPage(document) {
    ...
    document.getElementById("jvchat-risi-reduce-range").value = configuration["risibank_height"];
    document.getElementById("jvchat-risi-reduce-span").innerHTML = `${configuration["risibank_height"]} px`;
    document.getElementById("jvchat-risi-reduce-range").addEventListener("input", tryCatch(risibankReduceModeOption));
    const risiBankNotInstalled = !document.querySelector(".risibank-toggle");
    if (risiBankNotInstalled) {
        document.getElementById("jvchat-risi-reduce").classList.add("jvchat-hide");
    }
    adjustRisiMaxHeight(configuration["risibank_height"]);
...
function risibankReduceModeOption(event) {
    let maxHeight = parseInt(document.getElementById("jvchat-risi-reduce-range").value);
    document.getElementById("jvchat-risi-reduce-span").innerHTML = `${maxHeight} px`;
    configuration["risibank_height"] = maxHeight;
    saveConfig();
    adjustRisiMaxHeight(maxHeight);
}
...
function adjustRisiMaxHeight(maxHeight) {
    document.getElementById("bloc-formulaire-forum")?.style.setProperty("--jvchat-risi-max-height", `${maxHeight}px`);
}
...
function setTextareaHeight(plusOne) {
    ...
    // La feuille de style vient lire notre variable CSS pour bypasser le height de React sans conflit d'attributs.
...
//Transforme le HTML de JVChat en JVCode en cas de click sur le bouton citation.
...
// Listerner Global sur #jvchat-main avec delegation d'evenement au click.
...
function main() {
    ...
    if (document.querySelector(".buttonsNavbar")) return init();
    ...
    // Fallback timeout: if buttonsNavbar never appears abort
